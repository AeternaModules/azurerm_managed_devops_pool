...
output "managed_devops_pools_azure_devops_organization" {
  ...
  value       = { for k, v in azurerm_managed_devops_pool.managed_devops_pools : k => one(v.azure_devops_organization) if v.azure_devops_organization != null && length(v.azure_devops_organization) > 0 }
}
...
output "managed_devops_pools_identity" {
  ...
  value       = { for k, v in azurerm_managed_devops_pool.managed_devops_pools : k => one(v.identity) if v.identity != null && length(v.identity) > 0 }
}
...
output "managed_devops_pools_stateful_agent" {
  ...
  value       = { for k, v in azurerm_managed_devops_pool.managed_devops_pools : k => one(v.stateful_agent) if v.stateful_agent != null && length(v.stateful_agent) > 0 }
}
output "managed_devops_pools_stateless_agent" {
  ...
  value       = { for k, v in azurerm_managed_devops_pool.managed_devops_pools : k => one(v.stateless_agent) if v.stateless_agent != null && length(v.stateless_agent) > 0 }
}
...
output "managed_devops_pools_virtual_machine_scale_set_fabric" {
  ...
  value       = { for k, v in azurerm_managed_devops_pool.managed_devops_pools : k => one(v.virtual_machine_scale_set_fabric) if v.virtual_machine_scale_set_fabric != null && length(v.virtual_machine_scale_set_fabric) > 0 }
}
